...
const Warehouse: React.FC<WarehouseProps> = ({ user }) => {
  ...
  const handleSaveChanges = async () => {
    ...
    if (!editedDestination) {
      toast.warning(String(t("warehouse.enterDestination")), { position: "top-right", autoClose: 3000 });
      return;
    }

    const coordsString = await getCoordsFromAddress(editedDestination);
    if (!coordsString) {
      toast.error(String(t("warehouse.invalidLocation")), { position: "top-right", autoClose: 3000 });
      return;
    }
    ...
    <div className={styles.container}>
      ...
      {loading ? (
        ...
        <div className={styles.packagesContainer}>
          {packages.map((pkg) => (
            <div key={pkg._id} className={styles.packageCard} onClick={() => handlePackageClick(pkg)}>
              <h2 className={styles.packageTitle}>{pkg.name}</h2>
              <p className={styles.packageInfo}>{String(t("warehouse.description"))}: {pkg.description}</p>
              <p className={styles.packageInfo}>{String(t("warehouse.weight"))}: {pkg.weight} kg</p>
              <p className={styles.packageInfo}>{String(t("warehouse.size"))}: {pkg.size} cm³</p>
            </div>
          ))}
        </div>
      )}
      ...
      {selectedPackage && (
        <div className={styles.editModal}>
          ...
          {isLoaded && (
            <label>{String(t("warehouse.destination"))}:
          ...
          <label>{String(t("warehouse.deliveredAt"))}:
            <input
              type="date"
          ...
          <div className={styles.buttonGroup}>
            ...
            <button onClick={() => setSelectedPackage(null)}>{String(t("warehouse.cancel"))}</button>
          </div>
        </div>
      )}
    </div>
