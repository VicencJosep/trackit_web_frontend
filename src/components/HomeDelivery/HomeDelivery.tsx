    ...
    try {
      ...
      if (data.rows && data.rows[0] && data.rows[0].elements) {
        ...
        const totalDistance = elements.reduce(
          (acc: number, el: { distance: { value: number }; duration: { value: number } }) =>
              ...
              <div className={styles.headerActions}>
                {isEditingOrder ? (
                  <>
                    ...
                    >
                      {isSaving
                        ? '⏳ ' + String(t('homeDelivery.saving'))
                        : '💾 ' + String(t('homeDelivery.save'))}
                    </button>
                  </>
                ) : (
                  <button
            ...
            {packets.length === 0 ? (
              ...
              <ul className={styles.packetsList}>
                {packets.map((packet, index) => (
                  <li
                    key={packet._id}
                    className={`${styles.packetItem} ${getPriorityClass(index)}`}
                  >
                      ...
                      <div className={styles.packetDetails}>
                        <div className={styles.packetDetail}>
                          ...
                          <span className={styles.detailText}>
                            {packet.destination ?? String(t('homeDelivery.notSpecified'))}
                          </span>
                        </div>
                        {packet.description && (
                          <div className={styles.packetDetail}>
